...
where Members.joindate = (select max(t.joindate)
                          from country_club.Members t
                         )
...
the member name. */
SELECT DISTINCT Facilities.name court_name, CONCAT( CONCAT( Members.firstname,  ' ' ) , Members.surname ) member_name
...
select Facilities.name facility_name,
	   CONCAT( CONCAT( Members.firstname,  ' ' ) , Members.surname ) member_name,
